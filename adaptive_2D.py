# -*- coding: utf-8 -*-
"""
Created on Tue Jun 24 00:13:01 2025
...
# %% first attempt to add adaptation current to the 2D rate network
### investigate behavior and balance
### if interesting figure out how computation changes
...
for tt in range(lt-1):
    ...
    re_xy[:,:,tt+1] = re_xy[:,:,tt] + dt/tau_e*( -re_xy[:,:,tt] + phi(Wee*(ge_conv_re) + Wei*(gi_conv_ri) + mu_e + \
                                                g0*ae_xy[:,:,tt]) + stim*pattern)
    ri_xy[:,:,tt+1] = ri_xy[:,:,tt] + dt/tau_i*( -ri_xy[:,:,tt] + phi(Wie*(ge_conv_re) + Wii*(gi_conv_ri) + mu_i +\
                                                g0*ai_xy[:,:,tt]) )
    
    ## adaptive dynamics
    ae_xy[:,:,tt+1] = ae_xy[:,:,tt] + dt/tau_ae*(-ae_xy[:,:,tt] + re_xy[:,:,tt])
    ai_xy[:,:,tt+1] = ai_xy[:,:,tt] + dt/tau_ai*(-ai_xy[:,:,tt] + ri_xy[:,:,tt])
...
def sim_2D_EI(N, lt=lt, sigs=(sig_e, sig_i), rescale=1):
    ...
    g0 = 1.* rescale ### for adaptive strength
