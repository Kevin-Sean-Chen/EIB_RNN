...
def relu2D_step(re, ri, N, dt, npf, ntype, K, tau, u, J0, sigma, J2, J3):
    ...
    for _ in range(npf):
        ...
        mue = K**0.5 * (u0_expanded + J0[0, 0] * conv_re + J0[0, 1] * conv_ri)
...
def make_2D_directional_stim(N, lt, time_f, space_f, device='cpu'):
    x = torch.linspace(0, space_f * 2 * np.pi, N, device=device)  # Create a range for sine wave input
    phases = torch.linspace(0, 2 * np.pi, N, device=device)  # Phase offsets
    I_xyt = torch.zeros((N, N, lt), device=device)
    drift_series = torch.zeros(lt, device=device)
    for t in range(lt):
        temp = torch.stack([torch.sin(x + phase*1 + t * time_f * (2 * np.pi / N)) for phase in phases], dim=1)
        I_xyt[:, :, t] = temp / (torch.max(torch.abs(temp)) + 1e-8)
        if t > 0:
            drift_series[t] = np.sign(time_f) #drift_series[t - 1] + time_f/lt*10
    return I_xyt/1, drift_series
...
class Relu2DReservoirRNN(nn.Module):
    ...
    def forward(self, input_pattern):
        ...
        for t in range(T):
            ...
            u[0] = u[0]*1 + input_pattern[:, :, t].cpu().numpy()*10  # Add input 2D array at time t
            re, ri = relu2D_step(
...
# ipt_img, drift = make_2D_stim_with_rigid_shift(N, T, 0.1, 2/N*.5)  ### orginal rigid shift
space_f = 3*1 #3
time_f = .2*2
direction = +1
ipt_img, drift = make_2D_directional_stim(N, T, time_f*direction, space_f)
ipt_img = ipt_img.to(device)
target = drift.unsqueeze(0)  # [1, T]

print(np.max(ipt_img.cpu().numpy()), np.min(ipt_img.cpu().numpy()))

# visualize the input
plt.figure(figsize=(10, 5))
plt.subplot(1, 3, 1)
plt.imshow(ipt_img[:, :, 10].cpu().numpy(), cmap='gray')
plt.title('Input Pattern at t=10')
plt.subplot(1, 3, 2)
plt.imshow(ipt_img[:, :, 100].cpu().numpy(), cmap='gray')
plt.title('Input Pattern at t=100')
plt.subplot(1, 3, 3)
...
# Evaluation
ipt_img_plus, target_p = make_2D_directional_stim(N, T, time_f*1, space_f)
ipt_img_minus, target_n = make_2D_directional_stim(N, T, time_f*-1, space_f)
output_p, re_all_p = model(ipt_img_plus.to(device))
output_n, re_all_n = model(ipt_img_minus.to(device))
import matplotlib.pyplot as plt
plt.figure()
plt.plot(output_p.detach().cpu().numpy().squeeze(), label='readout +1')
plt.plot(output_n.detach().cpu().numpy().squeeze(), label='readout -1')
plt.plot(target_p.cpu().numpy().squeeze(), label='target +1')
plt.plot(target_n.cpu().numpy().squeeze(), label='target -1')
plt.legend()
plt.show()

### visualize three frames of the reservoir state, from initial, middle, and end
re_all = re_all_p.detach()  # [N, N, T]
