...
if __name__ == "__main__":
    ...
    reluRNN_params = {
        'dt': 0.001,
        'tau': 0.005,
        'J0': torch.randn(N, N, device=device, dtype=torch.float32) * (20 / math.sqrt(N*N)), ### tune this!
        'u': 0,
    }
    ...
    target_out, target_mem, space_stim, input_traj = make_wm_trial(N, T, delay_period=delay_period, cue_interval=cue_interval, lr_trial=None)
    ...
    for epoch in range(epochs):
        ...
        loss_out = criterion(output.view(-1), target_out.view(-1))
        loss_mem = criterion(mem.view(-1), target_mem.view(-1))

        loss = loss_out + alpha * loss_mem
        loss.backward()
        optimizer.step()
    ...
    for ii in range(n_trials):
        target_out, target_mem, space_stim, input_traj = make_wm_trial(N, T, delay_period=delay_period, cue_interval=cue_interval, lr_trial=0)
        output, mem, _ = model(space_stim)
        plt.subplot(2,1,1)
        ...
        target_out, target_mem, space_stim, input_traj = make_wm_trial(N, T, delay_period=delay_period, cue_interval=cue_interval, lr_trial=1)
        output, mem, _ = model(space_stim)
        plt.subplot(2,1,1)
