...
# %% 2D EI model comparison
# %% space vs. shuffle
EI_balanced = np.array([0.0921, 0.0687, 0.0566])
EI_imb = np.array([0.6338, 0.8569, 0.7542])
vRNN = np.array([0.4913, 0.8442, 1.17, 0.6316])  ### run with echo_RNN


plt.figure()
plt.bar([r"balance 2D" "\n" r"($<\beta>\sim 0.2$)", r"imbalance 2D" "\n" r"($<\beta>\sim0.5$)", "vanilla RNN"], 
        [np.mean(EI_balanced), np.mean(EI_imb), np.mean(vRNN)], \
            yerr =  [np.std(EI_balanced), np.std(EI_imb), np.std(vRNN)])
