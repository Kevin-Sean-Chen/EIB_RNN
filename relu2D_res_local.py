...
def relu2D_step(re, ri, N, dt, npf, ntype, K, tau, u, J0, sigma, J2, J3):
    ...
    we = np.sum(dx * (2 * np.pi * sigma[0]**2)**-0.5 *
                np.exp(-0.5 * (dx * (x[:, None] + k - bias*1))**2 / sigma[0]**2), axis=1)
    wi = np.sum(dx * (2 * np.pi * sigma[1]**2)**-0.5 *
                np.exp(-0.5 * (dx * (x[:, None] + k - bias*0))**2 / sigma[1]**2), axis=1)
...
class Relu2DReservoirRNN(nn.Module):
    def __init__(self, N, T, output_dim, device, relu2D_params, mask):
        super().__init__()
        self.N = N
        self.T = T
        ...
        # Readout weights
        self.mask = torch.tensor(mask, device=device, dtype=torch.float32)  # Convert mask to tensor and put on device
        num_zeros_in_mask = (self.mask == 0).sum().item()
        # self.W_out = nn.Parameter(torch.randn(num_zeros_in_mask, output_dim, device=device) * 0.1)
    ...
    def forward(self, input_pattern):
        ...
        N = self.N
        T = self.T
        # Initial state based on J0 and u
        # J0_inv = torch.tensor(np.linalg.inv(self.relu2D_params['J0']), device=self.device, dtype=torch.float32)
        # u_tensor = torch.tensor(np.array(self.relu2D_params['u']), device=self.device, dtype=torch.float32)
        # r0 = -J0_inv @ u_tensor
        # re = r0[0] + 0.05 * torch.rand(self.N, self.N, device=self.device, dtype=torch.float32)
        # ri = r0[1] + 0.08 * torch.rand(self.N, self.N, device=self.device, dtype=torch.float32)
        re = 0.05 * torch.rand(self.N, self.N, device=self.device, dtype=torch.float32)
        ri = 0.08 * torch.rand(self.N, self.N, device=self.device, dtype=torch.float32)
        re_all = []
        for t in range(T):
            # Add input as external drive to excitatory population
            u = self.relu2D_params['u']  # Read baseline u
            u[0] = u[0]*0 + input_pattern[:, :, t].cpu().numpy()** self.mask.cpu().numpy() *10*1  ##### Add input 2D array at time t
            # u[0] = u[0] * self.mask.cpu().numpy()
            re, ri = relu2D_step(
        ...
        # re_all = re_all.reshape(N*N, T)
        # readout = re_all[mask_flat == 0, :]
...
output_dim = 1
device = 'cpu'
mask = torch.ones((N, N))*0
mask[:N//2, :] = 1  # Mask out a central 7x7 region for readout
...
relu2D_params = {
    ...
    #'J0': np.array([[1, -1], [1, -1]]), #
    'J0': np.array([[1, -4], [2, -2]]),
    'sigma': 0.05 * np.array([1, np.sqrt(2)])*1,
    'J2': np.zeros((2,2)),
    'J3': np.zeros((2,2)),
}
...
ipt_img, drift = make_2D_stim_with_rigid_shift(N, T, 0.1, 2/N*.5*1)
...
model = Relu2DReservoirRNN(N, T, output_dim, device, relu2D_params, mask)
optimizer = optim.Adam(model.parameters(), lr=0.01*5)
