...
# tau_i, sig_i = 20*0.001, 0.12 ### waves!!!
...
Iamp = 1 *2.*(N**2*sig_e**2*np.pi*1)**0.5 *rescale / 1.  # same scale as the spontaneous input?
Iamp_i = 1 *2.*(N**2*sig_i**2*np.pi*1)**0.5 *rescale / 1. 
...
kernel_size = N*1-1 #23 #37  # pick this for numerical convolution
...
def spatial_convolution(r,k):
    ...
    # gr = convolve(r.squeeze(), k, mode='wrap')
    # gr = fftconvolve(r.squeeze(), k, mode='same')
    return gr
...
temp_k2 = g_kernel(5/N, N)
pattern2 = np.random.randn(N,N)
pattern2 = spatial_convolution(pattern2, temp_k2)
pattern2 = pattern2 - np.mean(pattern2)  ### remove baseline for zero net
pattern2 = pattern2 / np.max(np.abs(pattern2))  ### unit strength
spatial_input = pattern2*1
...
for tt in range(lt-1):
    ...
    gi_conv_ri = spatial_convolution(ri_xy[:,:,tt], g_kernel(sig_i))
    he_xy[:,:,tt+1] = he_xy[:,:,tt] + dt/tau_e*( -he_xy[:,:,tt] + (Wee*(ge_conv_re) + Wei*(gi_conv_ri) + mu_e \
                                                                   + 1*spatial_input*Iamp*y_test[tt]) )
    hi_xy[:,:,tt+1] = hi_xy[:,:,tt] + dt/tau_i*( -hi_xy[:,:,tt] + (Wie*(ge_conv_re) + Wii*(gi_conv_ri) + mu_i \
                                                                   + 1*pattern*Iamp_i*y_test[tt]) )
    re_xy[:,:,tt+1] = phi(he_xy[:,:,tt+1])
    ri_xy[:,:,tt+1] = phi(hi_xy[:,:,tt+1])
